...
import { generateTwitterVerifierCircuitInputs } from "@zk-email/helpers";
...
export async function generateTwitterVerifierCircuitInputs() {
    ...
    const emailVerifierInputs = generateCircuitInputs({
        ...
        pubkey: dkimResult.pubkey,
        emailBody: dkimResult.emailBody,
        bodyHashIndex:dkimResult.bodyHashIndex,
        emailHeader: dkimResult.emailHeader,
        precomputedSHA: STRING_PRESELECTOR,
        emailHeaderLength: MAX_HEADER_PADDED_BYTES,
        emailBodyLength: MAX_BODY_PADDED_BYTES
    });
...
To build your circom circuit, you need to import a couple files. The first one is the `email-verifier.circom` from our library. The second one is the `twitter_reset_regex.circom` file that you generated earlier in this tutorial. Latly, you need `from_addr_regex.circom`.
...
`exposeFrom - This is the number of bytes that can fit into a 255ish bit signal (can increase later).
...
Similarly, the Twitter username verification uses a regex pattern to scan the email body.
