...
To build your circom circuit, you need to import a couple of files. The first one is the `email-verifier.circom` from our library. The second one is the `twitter_reset_regex.circom` file that you generated earlier in this tutorial. Lastly, you need `from_addr_regex.circom`.
...
The `email-verifier.circom` file validates each of the inputs to the circuit, while the `twitter_reset_regex.circom` file scans the email body for the '@username', ensuring it adheres to the specified regex definition. The `from_addr_regex.circom` uses regex to verify the "from" address in an email.
...
`maxHeaderLength = 1024` - This is the maximum number of bytes in the header.

`maxBodyLength = 1536` - This is the maximum number of bytes in the body after the precomputed slice.
...
`exposeFrom - This is the number of bytes that can fit into a 255-ish bit signal (can increase later).
...
`emailHeader[maxHeadersLength]` - Prehashed email data.

`pubkey[k]` - RSA public key.

`signature[k]` - RSA signature.

`emailBody[maxBodyLength]` - Length of the email body, including padding.

`address` - Wallet address.

`bodyHashIndex` - Index of the body hash.

`twitterUsernameIndex` - Index of Twitter username.

**Outputs**

`pubkeyHash` - Hash of the public key, cross-referenced with the public key in our DKIM registry to authenticate the origins of the email.

`twitterUsername` - Packed Twitter username that has been verified and is ready for reveal.
...
### Step 4: Regex verification of the "From" address and Twitter username

This section explains how we utilize regular expressions (regex) to verify both the "From" email address and the Twitter username found within the email body.
...
In this code snippet, `FromAddrRegex` is applied to the `emailHeader` to search for the "From" address. The result is a binary signal `fromEmailFound` which indicates whether the regex pattern was successfully matched (`1` for found, `0` for not found). If found, the `fromEmailReveal` array contains the characters of the "From" address extracted from the header.

Following the detection, the "From" address is then packed into a more compact format for efficient handling within the circuit:
...
Similarly, Twitter username verification uses a regex pattern to scan the email body.
...
Here, `TwitterResetRegex` is used to parse the `emailBody`. The `twitterFound` signal indicates the presence of a Twitter username. If the username is found, `twitterReveal` will contain the characters of the Twitter handle.

Once the Twitter username is identified, it is packed into an integer format to ensure it fits within a single field element of the circuit, optimizing both space and verification speed:
...
```
6. **Export the verification key**: This step exports the verification key from the Twitter circuit. The verification key is used to verify proofs.
```bash
...
You can create an `inputs.ts` file and place this code inside:
``` typescript
...
```
To create the inputs.json file you can create a `generate-inputs.ts` script:
...
To generate inputs run:

```bash
ts-node generate-proof.ts --email-file ../tests/emls/twitter-test.eml --ethereum-address <your-eth-address>
```
Remember to replace the address and the path to the raw email file with your own.
